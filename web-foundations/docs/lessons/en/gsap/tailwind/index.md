...
lang: en
permalink: /lessons/en/gsap/tailwind/
---
...
For this course, we use CDN to focus on learning GSAP's API without build tool complexity. All demos use **Tailwind CSS v4** via CDN for styling.
...
<body class="min-h-screen grid place-items-center font-sans bg-zinc-950 text-zinc-50 overflow-x-hidden">
  <h1 class="hero-title text-[clamp(3rem,10vw,8rem)] font-bold tracking-tight">Hello GSAP</h1>
...
> **⚠️ Important:** All demos use `class="h-full overflow-x-hidden"` on the `<html>` element and `overflow-x-hidden` on the `<body>` element to prevent horizontal scroll issues when embedded in lesson pages. This is **critical** for proper scroll management in student examples. Always include these classes when creating embedded demos.

### Tailwind CSS v4 Quick Reference

All examples in this lesson use **Tailwind CSS v4** via CDN. Key patterns:

```html
<!-- Always include Tailwind 4 CDN in <head> -->
<script src="https://cdn.tailwindcss.com"></script>
...
**Common Tailwind 4 utility classes used in GSAP demos:**
- Layout: `flex`, `grid`, `items-center`, `justify-center`
- Spacing: `p-8`, `m-4`, `gap-4`
- Typography: `text-[clamp(2rem,8vw,6rem)]`, `font-bold`, `tracking-tight`
- Colors: `bg-zinc-950`, `text-zinc-50`, `border-zinc-800`
- Effects: `overflow-hidden`, `rounded-xl`, `shadow-lg`
...
```html
<h1 class="headline text-[clamp(2rem,8vw,6rem)] font-extrabold overflow-hidden">
  <span class="word inline-block mr-1 overflow-hidden">Design</span>
  <span class="word inline-block mr-1 overflow-hidden">with</span>
  <span class="word inline-block mr-1 overflow-hidden">intention</span>
</h1>
```
...
```html
<h1 class="split-text text-[clamp(3rem,12vw,9rem)] font-black tracking-tight">Typography</h1>
```
...
<div class="flex items-center justify-center">
  <svg viewBox="0 0 200 200" width="200" height="200" class="w-50 h-50">
...
```html
<div class="flex items-center justify-center">
  <svg viewBox="0 0 400 200" width="400" height="200" class="w-full max-w-2xl">
